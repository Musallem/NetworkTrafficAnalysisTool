...
TCP_model.feature_names = ['tcp_seq', 'tcp_ack','tcp_flags_syn', 'tcp_flags_ack', 'tcp_checksum']

# Write model path
IP_model = joblib.load('rfmodel_ip_id_p15.joblib')
IP_model.feature_names = ['ip_id', 'ip_ttl', 'ip_len', 'ip_flags_df']
...
class MainWindow(QWidget, From_Main):
    ...
    def OpenFile(self):
        try:
            ...
            tcp_X = pd.DataFrame(columns=['tcp_seq', 'tcp_ack','tcp_flags_syn', 'tcp_flags_ack', 'tcp_checksum'])
            ip_X = pd.DataFrame(columns=['ip_id', 'ip_ttl', 'ip_len', 'ip_flags_df'])
            ...
            if not tcp_X.empty:
                imputer = KNNImputer()
                tcp_X[['tcp_seq', 'tcp_ack','tcp_flags_syn', 'tcp_flags_ack', 'tcp_checksum']] = imputer.fit_transform(tcp_X[['tcp_seq', 'tcp_ack','tcp_flags_syn', 'tcp_flags_ack', 'tcp_checksum']])

            class_label = {0: "covert", 1: "normal"}

            if not ip_X.empty:
                imputer = KNNImputer()
                ip_X[['ip_id', 'ip_ttl', 'ip_len', 'ip_flags_df']] = imputer.fit_transform(ip_X[['ip_id', 'ip_ttl', 'ip_len', 'ip_flags_df']])

            if not tcp_X.empty:
                tcp_predictions = TCP_model.predict(tcp_X)
                print(tcp_predictions)
                tcp_prediction_probs = TCP_model.predict_proba(tcp_X)
                print(tcp_prediction_probs)
